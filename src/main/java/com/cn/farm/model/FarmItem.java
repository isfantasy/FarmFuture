...
public class FarmItem {
    ...
    public FarmItem(String name){
        ...
        for (JsonNode feed:feedArray){
            ...
            feed1.setLevel(Integer.parseInt(feed.get("level").toString()));
            feedList.add(feed1);
        }
        for (JsonNode muck:muckArray){
            ...
            muck1.setLevel(Integer.parseInt(muck.get("level").toString()));
            muck1.setEffectHour(Integer.parseInt(muck.get("effectHour").toString()));
            muckList.add(muck1);
        }
    }
